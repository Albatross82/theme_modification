...
function nn_deregister_styles() {
			wp_enqueue_style('theme_styles5', plugin_dir_url(__FILE__) . 'css/theme_styles_mpp.css');
		}
add_action( 'wp_print_styles', 'nn_deregister_styles', 100 );

function pp_deregister_style2() {
	$uri_parts = explode('/', $_SERVER['REQUEST_URI']);
    if (in_array('?projectref', explode('=', $uri_parts[3]))) {
	    wp_enqueue_style('projects_styles2', plugin_dir_url(__FILE__) . 'css/shop_styles_mpp.css');	}
    else {
    }
}
...
function myplugin_woocommerce_locate_template( $template, $template_name, $template_path ) {
  ...
  $plugin_path  = myplugin_plugin_path() . '/templates/woocommerce_mpp/';
...
function my_admin_logo() {
   echo '
    <style type="text/css">
        #header-logo { background:url(https://myphotopages.ru/favicon.ico) no-repeat 0 0 !important; }
    </style>';
}
...
function my_login_logo(){
   echo '
   <style type="text/css">
        #login h1 a { background: url(https://myphotopages.ru/wp-content/uploads/2016/07/logo1sm.png) no-repeat 0 0 !important; width: 150px; }
    </style>';
}
